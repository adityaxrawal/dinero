...
describe('test_paid_mau_counts_only_active_in_window', () => {
  it('counts only active-status subscriptions', async () => {
    const count = vi
      .fn()
      .mockImplementation(({ where }) => Promise.resolve(where.status === 'active' ? 42 : 0));
...
describe('test_conversion_rate_calculation', () => {
  it('computes converted/started within the window', async () => {
    const count = vi
      .fn()
      .mockImplementation(({ where }) => Promise.resolve(where.status === 'active' ? 3 : 10));
    const db = { subscription: { count } } as unknown as Pick<MetricsDb, 'subscription'>;
...
describe('test_churn_rate_calculation', () => {
  it('computes churned/active-at-start', async () => {
    const count = vi
      .fn()
      .mockImplementation(({ where }) => Promise.resolve(where.status === 'canceled' ? 5 : 100));
    const db = { subscription: { count } } as unknown as Pick<MetricsDb, 'subscription'>;
      ...
      plan: {
        findUnique: vi
          ...
          .mockImplementation(({ where }) =>
            Promise.resolve(
              where.id === 'desktop_pro_monthly' ? { amountMinor: 29900 } : { amountMinor: 287040 }
            )
          ),
