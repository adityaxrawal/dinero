...
function makeDb(subscription: Record<string, unknown> | null) {
  ...
  const db: CancelDb = {
    subscription: { findFirst: vi.fn().mockResolvedValue(subscription), update } as unknown as CancelDb['subscription'],
    licensingAuditLog: { create: vi.fn().mockResolvedValue({}), findMany: vi.fn().mockResolvedValue([]) } as unknown as CancelDb['licensingAuditLog'],
  };
  return { db, update };
}
...
describe('test_cancel_at_period_end_retains_access_until_period_ends', () => {
  it('defaults to cancel-at-period-end, keeping status unchanged (not immediately locked)', async () => {
    const { db, update } = makeDb({ id: 'sub_1', accountId: 'acc_1', status: 'active', currentPeriodEnd: new Date(Date.now() + 86400000) });
  ...
  it('immediate cancellation (cancel_at_period_end=false) sets status to canceled now', async () => {
    const { db } = makeDb({ id: 'sub_1', accountId: 'acc_1', status: 'active', currentPeriodEnd: new Date(Date.now() + 86400000) });
    const result = await cancelSubscription(db, { account_id: 'acc_1', cancel_at_period_end: false });
...
describe('test_reactivation_before_period_end_undoes_cancellation', () => {
  it('clears cancel_at_period_end when the period has not ended yet', async () => {
    const { db, update } = makeDb({ id: 'sub_1', accountId: 'acc_1', status: 'active', cancelAtPeriodEnd: true, currentPeriodEnd: new Date(Date.now() + 86400000) });
    const result = await reactivateSubscription(db, 'acc_1');
    expect(result.status).toBe('reactivated');
    expect(update).toHaveBeenCalledWith(expect.objectContaining({ data: { cancelAtPeriodEnd: false } }));
  });

  it('rejects reactivation once the period has already ended', async () => {
    const { db } = makeDb({ id: 'sub_1', accountId: 'acc_1', status: 'canceled', cancelAtPeriodEnd: true, currentPeriodEnd: new Date(Date.now() - 86400000) });
    await expect(reactivateSubscription(db, 'acc_1')).rejects.toMatchObject({ code: 'VALIDATION_ERROR' });
  });
});
