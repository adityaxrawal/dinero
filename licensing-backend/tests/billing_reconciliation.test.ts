...
function makeDb(subscriptions: { id: string; accountId: string; status: string }[], records: Record<string, string>) {
  const update = vi.fn().mockResolvedValue({});
  const db: ReconciliationDb = {
    subscription: { findMany: vi.fn().mockResolvedValue(subscriptions), update } as unknown as ReconciliationDb['subscription'],
    paymentProviderRecord: {
      findFirst: vi.fn().mockImplementation(({ where }) =>
        Promise.resolve(records[where.subscriptionId] ? { razorpaySubscriptionId: records[where.subscriptionId] } : null)
      ),
    } as unknown as ReconciliationDb['paymentProviderRecord'],
    licensingAuditLog: { create: vi.fn().mockResolvedValue({}), findMany: vi.fn().mockResolvedValue([]) } as unknown as ReconciliationDb['licensingAuditLog'],
  };
  return { db, update };
}
...
describe('test_drift_detected_and_corrected', () => {
  it('corrects a subscription whose local status disagrees with Razorpay', async () => {
    const { db, update } = makeDb([{ id: 'sub_1', accountId: 'acc_1', status: 'active' }], { sub_1: 'rzp_sub_1' });
...
describe('test_correction_logged_to_audit_not_silent', () => {
  it('logs before/after status and the razorpay status observed', async () => {
    const { db } = makeDb([{ id: 'sub_1', accountId: 'acc_1', status: 'active' }], { sub_1: 'rzp_sub_1' });
    ...
    expect(db.licensingAuditLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          ...
          payload: expect.objectContaining({ local_status_was: 'active', corrected_to: 'canceled' }),
        }),
      })
    );
  });
...
describe('test_reconciliation_summary_report_generated', () => {
  it('reports checked/drifted/corrected counts across multiple subscriptions', async () => {
    ...
    const razorpay = { fetch: vi.fn().mockImplementation((id) => Promise.resolve({ status: id === 'rzp_sub_1' ? 'halted' : 'active' })) };
    const summary = await runBillingReconciliation(db, razorpay);
