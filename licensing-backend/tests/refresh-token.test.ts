...
function makeDb(
  ...
  return {
    ...
    } as unknown as RefreshDb['licenseToken'],
    subscription: {
      findFirst: vi.fn().mockResolvedValue(subscription),
    } as unknown as RefreshDb['subscription'],
  };
}
...
describe('test_refresh_issues_new_jwt_with_extended_expiry', () => {
  it('returns a fresh JWT with a later expiry than the original', async () => {
    ...
    const result = await refreshLicenseToken(
      db,
      { jwt: original, device_id: 'device-A' },
      TEST_PUBLIC_KEY_PEM,
      TEST_PRIVATE_KEY_PEM
    );
    expect(result.status).toBe('refreshed');
    const originalClaims = verifyLicenseJwt(original, TEST_PUBLIC_KEY_PEM, {
...
describe('test_refresh_rejects_hardware_uuid_mismatch', () => {
  it('rejects when the requesting device does not match the JWT device_id claim', async () => {
    ...
    const db = makeDb(
      { id: 'lt_1', accountId: 'acc_1', deviceFingerprint: 'device-A' },
      { planId: 'x', billingInterval: 'monthly', status: 'active' }
    );
    await expect(
      refreshLicenseToken(
        db,
        { jwt: original, device_id: 'device-DIFFERENT' },
        TEST_PUBLIC_KEY_PEM,
        TEST_PRIVATE_KEY_PEM
      )
...
describe('test_refresh_rejects_beyond_max_staleness', () => {
  it('rejects a token that expired more than 48 hours ago', async () => {
    ...
    await expect(
      refreshLicenseToken(
        db,
        { jwt: staleToken, device_id: 'device-A' },
        TEST_PUBLIC_KEY_PEM,
        TEST_PRIVATE_KEY_PEM
      )
  ...
  it('allows a token expired less than 48 hours ago', async () => {
    ...
    await expect(
      refreshLicenseToken(
        db,
        { jwt: recentlyExpired, device_id: 'device-A' },
        TEST_PUBLIC_KEY_PEM,
        TEST_PRIVATE_KEY_PEM
      )
