...
describe('test_trial_blocked_for_reused_email', () => {
  it('blocks when the account has already used its trial and the device is unknown', () => {
    const decision = decideTrialEligibility({ accountTrialUsed: true, deviceBoundSubscriptionStatus: null, deviceHasPriorTrialStartedEvent: false });
...
describe('test_trial_blocked_for_reused_device_fingerprint', () => {
  it('blocks when this device already started a trial, regardless of email', () => {
    const decision = decideTrialEligibility({ accountTrialUsed: false, deviceBoundSubscriptionStatus: null, deviceHasPriorTrialStartedEvent: true });
...
describe('test_os_reinstall_recognized_not_blocked_as_abuse', () => {
  it('recognizes a device already bound to an active (non-trial) subscription as returning, not abuse', () => {
    const decision = decideTrialEligibility({ accountTrialUsed: true, deviceBoundSubscriptionStatus: 'active', deviceHasPriorTrialStartedEvent: true });
    expect(decision).toEqual({ outcome: 'recognized_returning_device', existingSubscriptionStatus: 'active' });
  });
  ...
  it('still treats a device bound only to a trialing subscription as an ordinary device-reuse block', () => {
    ...
    const decision = decideTrialEligibility({ accountTrialUsed: true, deviceBoundSubscriptionStatus: 'trialing', deviceHasPriorTrialStartedEvent: true });
    expect(decision.outcome).toBe('blocked_device_reused');
  });

  it('allows a genuinely new device with a genuinely new email', () => {
    const decision = decideTrialEligibility({ accountTrialUsed: false, deviceBoundSubscriptionStatus: null, deviceHasPriorTrialStartedEvent: false });
