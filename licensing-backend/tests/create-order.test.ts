...
describe('test_order_creation_returns_valid_config', () => {
  it('returns order id/amount/currency/key_id for an active plan', async () => {
    const db = makeDb(
      { id: 'desktop_pro_monthly', isActive: true, amountMinor: 29900, currency: 'INR' },
      { id: 'acc_1', email: 'user@example.com' }
    );
    const razorpay = {
      create: vi.fn().mockResolvedValue({ id: 'order_abc', amount: 29900, currency: 'INR' }),
    };
    const result = await createOrder(
      db,
      razorpay,
      { email: 'user@example.com', plan_id: 'desktop_pro_monthly' },
      'rzp_test_key'
    );
  ...
  it('finds-or-creates the account when this is a first-time purchaser', async () => {
    ...
    await createOrder(
      db,
      razorpay,
      { email: 'new@example.com', plan_id: 'desktop_pro_monthly' },
      'rzp_test_key'
    );
  ...
  it('rejects an inactive plan', async () => {
    const db = makeDb(
      { id: 'x', isActive: false, amountMinor: 1, currency: 'INR' },
      { id: 'acc_1', email: 'user@example.com' }
    );
    const razorpay = { create: vi.fn() };
    await expect(
      createOrder(db, razorpay, { email: 'user@example.com', plan_id: 'x' }, 'rzp_test_key')
...
describe('test_client_side_success_claim_rejected_without_valid_signature', () => {
  ...
  it('rejects a signature computed with the wrong secret', () => {
    const orderId = 'order_abc';
    const paymentId = 'pay_123';
    const wrongSecretSignature = createHmac('sha256', 'not-the-real-secret')
      .update(`${orderId}|${paymentId}`)
      .digest('hex');
