// Doc 30 TASK-LIC-007 acceptance criteria.
...
function makeDb(opts: { existingAccount?: Record<string, unknown> | null; priorDeviceTrialAuditRows?: unknown[] }): StartTrialDb {
  ...
  return {
    ...
    subscription: { create: vi.fn().mockResolvedValue({}) } as unknown as StartTrialDb['subscription'],
    licenseToken: { create: vi.fn().mockResolvedValue({}) } as unknown as StartTrialDb['licenseToken'],
    licensingAuditLog: {
      create: vi.fn().mockImplementation(({ data }) => {
        auditRows.push({ eventType: data.eventType, payload: data.payload, createdAt: new Date() });
        return Promise.resolve({});
      }),
      // Mirrors real Prisma behavior: filters by the requested eventType,
      // unlike a naive mock that returns every accumulated row regardless.
