...
test.describe('Statement Management - Rigorous Verification', () => {
  ...
  test('should rigorously handle password prompt: timeouts, incorrect retry, and success', async ({ page }) => {
    ...
    await page.evaluate(() => { (window as unknown as { __MOCK_STATE__: Record<string, boolean> }).__MOCK_STATE__.password_failure = true; });
    ...
    await page.evaluate(() => { (window as unknown as { __MOCK_STATE__: Record<string, boolean> }).__MOCK_STATE__.password_failure = false; });
