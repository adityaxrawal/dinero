...
test.describe('Accessibility Compliance - Rigorous Verification', () => {
  ...
  // Appearance, LLM Config, Privacy/Consent History, Pattern Rules,
  // Statement Passwords, Data Management) -- was never once included in the
  // automated scan despite every other top-level route being covered since
  // this file's introduction. Network Activity moved to /#/debug (Document
  // 11 §17/Document 36 §3's already-documented location) and is no longer
  // part of this page.
  //
  // /#/debug added when Network Activity moved there -- previously never
  // scanned at all despite being a real, user-facing operational page.
  const pagesToTest = ['/', '/#/transactions', '/#/statements', '/#/reconciliation', '/#/instruments', '/#/spending-limits', '/#/onboarding', '/#/settings', '/#/debug'];
  ...
  test('modals enforce proper aria roles and escape mechanisms', async ({ page }) => {
    ...
    await page.waitForFunction(() => (window as unknown as { __TAURI_LISTENERS__: Record<string, unknown> }).__TAURI_LISTENERS__ && (window as unknown as { __TAURI_LISTENERS__: Record<string, unknown> }).__TAURI_LISTENERS__['db_corrupted']);
