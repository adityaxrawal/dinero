...
test.describe('AppShell & Navigation - Rigorous Verification', () => {
  ...
  test('should display global background task indicator without blocking UI', async ({ page }) => {
    ...
    // Wait for listener
    await page.waitForFunction(() => (window as unknown as { __TAURI_LISTENERS__: Record<string, unknown> }).__TAURI_LISTENERS__ && (window as unknown as { __TAURI_LISTENERS__: Record<string, unknown> }).__TAURI_LISTENERS__['background_task_progress']);
  ...
  test('should show a persistent, non-dismissable blocking overlay on Keychain denial', async ({ page }) => {
    await expect(page.locator('aside')).toBeVisible();
    await page.waitForFunction(() => (window as unknown as { __TAURI_LISTENERS__: Record<string, unknown> }).__TAURI_LISTENERS__ && (window as unknown as { __TAURI_LISTENERS__: Record<string, unknown> }).__TAURI_LISTENERS__['system_warning']);
