...
test.describe('Onboarding Flow - Rigorous Verification', () => {
  ...
  test('should enforce timezone, boundary limits, and strictly gate completion', async ({ page }) => {
    ...
    await page.evaluate(() => {
      (window as unknown as { __MOCK_STATE__: Record<string, boolean> }).__MOCK_STATE__.gmail_failure = true;
    });
    ...
    // Mock success
    await page.evaluate(() => {
      (window as unknown as { __MOCK_STATE__: Record<string, boolean> }).__MOCK_STATE__.gmail_failure = false;
    });
