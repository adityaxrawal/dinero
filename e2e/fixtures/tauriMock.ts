...
const tauriMockInitScript = `
  ...
  window.__TAURI_INTERNALS__.invoke = async function (cmd, args) {
    ...
    if (cmd === 'llm_get_available_models') {
      ...
      return [
        { id: 'gemma4_e4b', name: 'Gemma 4 E4B', tag: 'gemma4:e4b', tier: 1, min_ram_gb: 8.0, approx_size_gb: 5.0, rationale: 'The floor for customers who can't run anything bigger. Native structured JSON output and function calling.', gguf_url: '', expected_sha256: '' },
        { id: 'gemma4_12b', name: 'Gemma 4 12B', tag: 'gemma4:12b', tier: 2, min_ram_gb: 16.0, approx_size_gb: 9.0, rationale: 'The default and sweet spot. Highly capable on 16GB, quality matching models twice its size.', gguf_url: '', expected_sha256: '' },
        { id: 'qwen3_6_27b', name: 'Qwen3.6-27B (Dense)', tag: 'qwen3.6:27b', tier: 3, min_ram_gb: 16.0, approx_size_gb: 15.0, rationale: 'Strong alternative/cross-check to Gemma 12B. Dense architecture means it is consistent and predictable.', gguf_url: '', expected_sha256: '' },
        { id: 'qwen3_6_35b_a3b', name: 'Qwen3.6-35B-A3B (MoE)', tag: 'qwen3.6:35b', tier: 4, min_ram_gb: 32.0, approx_size_gb: 21.0, rationale: 'Highest accuracy ceiling realistic for a consumer Mac. MoE makes it fast despite size.', gguf_url: '', expected_sha256: '' },
        { id: 'gemma4_31b', name: 'Gemma 4 31B (Dense)', tag: 'gemma4:31b', tier: 5, min_ram_gb: 32.0, approx_size_gb: 20.0, rationale: 'Currently the strongest Gemma 4 model. Pick it for maximum quality if memory allows.', gguf_url: '', expected_sha256: '' },
      ];
    }
    if (cmd === 'llm_get_downloaded_models') return window.__MOCK_STATE__.llm_downloaded_models || [];
    if (cmd === 'llm_get_active_model') return window.__MOCK_STATE__.llm_active_model || 'gemma4_12b';
    if (cmd === 'llm_set_active_model') {
      window.__MOCK_STATE__.llm_active_model = args.modelId;
      return null;
    }
    if (cmd === 'llm_download_model') {
      window.__MOCK_STATE__.llm_downloaded_models = [
        ...(window.__MOCK_STATE__.llm_downloaded_models || []),
        args.modelId,
      ];
      return null;
    ...
    if (cmd === 'settings_get_network_activity') {
      // Real command now takes { page, pageSize } and returns
      // { entries, meta: { page, page_size, total } } -- moved to the
      // Debug > Network Activity tab (NetworkActivityViewer.tsx) with real
      // pagination, previously returned every row unconditionally with no
      // meta at all.
      if (window.__MOCK_STATE__.network_activity_failure) {
        throw { code: 'NETWORK_ERROR', message: 'Failed to fetch network activity.' };
      }
      const all = window.__MOCK_STATE__.network_activity || [];
