...
vi.mock('@/hooks/mutations/useCommitStatementDraft', () => ({
  useCommitStatementDraft: () => ({ mutate: commit, isPending: isSaving }),
}));
...
describe('StatementReviewModal once staged', () => {
  ...
  it('shows a saving label and blocks a second submit while committing', async () => {
    isSaving = true;
    render(<StatementReviewModal />);
...
describe('StatementReviewModal dismissal', () => {
  it('discards the draft when dismissed with Escape, not just via Cancel', async () => {
    // Closing the dialog by any route has to go through handleCancel --
    // dropping out of the modal while leaving the draft staged would strand
    // it with no way back to the review UI.
    render(<StatementReviewModal />);
    await screen.findByLabelText('Bank name');

    fireEvent.keyDown(screen.getByRole('dialog'), { key: 'Escape' });

    await waitFor(() => expect(discard).toHaveBeenCalledWith('d1', expect.anything()));
    expect(closeReviewModal).toHaveBeenCalled();
  });
});
