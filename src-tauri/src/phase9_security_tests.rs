...
use crate::licensing::client::{LicensingClient, ActivateRequest};
use chrono::Utc;
...
fn test_license_state_machine_transitions() {
    ...
    assert_eq!(fetched_grace.subscription_status_cached, LicenseStatus::Grace);

    // Transition to locked
    transition_to_locked(&conn, false).unwrap();

    let fetched_locked = get_license_state(&conn).unwrap().unwrap();
    assert_eq!(fetched_locked.subscription_status_cached, LicenseStatus::Locked);
}
...
async fn test_licensing_backend_receives_no_financial_data() {
    ...
    let mock = server.mock("POST", "/api/license/activate")
