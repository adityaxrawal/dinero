use chrono::Utc;
use crate::licensing::state::{get_license_state, upsert_license_state, transition_to_locked, LicenseStateRow, LicenseStatus};
...
fn test_pdf_passwords_not_in_plaintext() {
    ...
    let mut stmt = conn.prepare("SELECT password_ciphertext FROM pdf_passwords WHERE id = 'pw1'").unwrap();
    let pwd: String = stmt.query_row([], |row| row.get(0)).unwrap();
    
    assert_ne!(pwd, "plaintext_password", "PDF password should not be stored in plaintext");
    
    let columns: Vec<String> = conn.prepare("PRAGMA table_info(pdf_passwords)").unwrap()
            ...
            for entry in std::fs::read_dir(dir).unwrap() {
                ...
                if path.is_dir() {
                    ...
                    for (i, line) in content.lines().enumerate() {
                        let l = line.replace(" ", "");
                        if l.contains(".execute(format!(") || l.contains(".query(format!(") || l.contains(".execute(&format!(") || l.contains(".query(&format!(") {
                            panic!("Found unparameterized query in file {}:{}: {}", path.display(), i+1, line);
                        }
                    }
                }
            }
        ...
        if dir.is_dir() {
            for entry in std::fs::read_dir(dir).unwrap() {
                ...
                if path.is_dir() {
                    ...
                    let content = std::fs::read_to_string(&path).unwrap();
                    assert!(!content.contains("pub token: String"), "IPC response should not contain token field");
                    assert!(!content.contains("pub password: String"), "IPC response should not contain password field");
                }
            }
        }
    }
...
fn test_gmail_tokens_in_keychain_only() {
    let oauth_file = std::env::current_dir().unwrap().join("src/ingestion/oauth.rs");
    if oauth_file.exists() {
        ...
        assert!(content.contains("keyring") || content.contains("Entry::new") || !content.contains("std::fs::write"), "Gmail tokens must be stored in macOS Keychain only, not on disk");
        assert!(!content.contains("File::create"), "Gmail tokens must not be written to disk");
    }
}
...
fn test_sqlite_encryption_key_in_keychain_only() {
    ...
    if db_mod_path.exists() {
        ...
        assert!(content.contains("keyring") || content.contains("Entry::new") || content.contains("password"), "Database initialization must use encryption key from Keychain");
    }
}
                ...
                for (i, line) in lines.iter().enumerate() {
                    if line.contains("#[tauri::command]") {
                        if i + 1 < lines.len() {
                            ...
                            assert!(!next_line.contains("serde_json::Value"), "IPC arguments must use strong Rust types, found Value in {}", path.display());
                        }
                    }
                }
            }
...
fn test_file_upload_validation_mime_size() {
    let commands_data = std::env::current_dir().unwrap().join("src/commands/data.rs");
    if commands_data.exists() {
        ...
        if content.contains("upload") || content.contains("File::") {
            assert!(content.contains("size") || content.contains("len") || content.contains("metadata().len()"), "File upload must validate file size");
            // Mime or magic bytes should be checked
            assert!(content.contains("application/pdf") || content.contains("magic"), "File upload must validate file type");
        }
    }
}
...
fn test_oauth_callback_strict_validation() {
    ...
    if oauth_file.exists() {
        ...
        if content.contains("callback") || content.contains("exchange") {
            assert!(content.contains("state") || content.contains("csrf"), "OAuth callback must strictly validate the state parameter");
        }
    }
}
