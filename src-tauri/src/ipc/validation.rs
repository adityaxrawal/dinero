...
pub fn validate_uuid(field_name: &str, value: &str) -> Result<(), AppError> {
    uuid::Uuid::parse_str(value).map_err(|_| {
        AppError::Validation(format!(
            "{field_name} must be a valid UUID, got '{value}'"
        ))
    })?;
    Ok(())
}
...
pub fn validate_date_range(
    ...
    let start = NaiveDate::parse_from_str(start_date, "%Y-%m-%d")
        .map_err(|_| AppError::Validation(format!("start_date '{start_date}' is not a valid YYYY-MM-DD date")))?;
    let end = NaiveDate::parse_from_str(end_date, "%Y-%m-%d")
        .map_err(|_| AppError::Validation(format!("end_date '{end_date}' is not a valid YYYY-MM-DD date")))?;
    if start > end {
        return Err(AppError::Validation(format!(
...
pub fn validate_amount_minor(field_name: &str, amount_minor: i64, allow_negative: bool) -> Result<(), AppError> {
...
mod tests {
    ...
    fn test_date_range_validation_rejects_inverted() {
        ...
        assert!(validate_date_range("2026-01-01", "2026-01-01").is_ok(), "equal dates are a valid single-day range");
    ...
    fn test_pagination_bounds_enforced() {
        ...
        assert!(validate_pagination(1, 201).is_err(), "per_page must be <= 200");
    ...
    fn test_negative_amount_rejected_where_invalid() {
        ...
        assert!(validate_amount_minor("amount_minor", -100, true).is_ok(), "allow_negative fields must accept a negative value");
        assert!(validate_amount_minor("amount_minor", MAX_REASONABLE_AMOUNT_MINOR + 1, false).is_err(), "unreasonably large amounts must be rejected");
    }
