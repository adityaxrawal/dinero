...
fn test_llm_prompt_generation() {
    ...
    assert!(
        ...
        "Prompt must not contain user profile data"
    );
}
...
#[tokio::test]
async fn test_llm_extraction_falls_back_gracefully_on_oom() {
    // We simulate a missing model/server by passing a non-existent app_dir
    // and model id. llama_sidecar's ensure_server_ready fails fast (no
