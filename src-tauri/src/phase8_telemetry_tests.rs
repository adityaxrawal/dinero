...
use crate::commands::debug::{
    debug_get_pipeline_state, debug_set_gmail_poll_paused, debug_set_scan_queue_paused, PatternRuleHealth,
};
use crate::db;
use crate::db::audit_log::{AuditLogRow, insert as insert_audit_log};
...
async fn setup_test_db() -> Pool {
    ...
    crate::db::init_db(db_path.clone()).await.expect("init test db")
}
...
async fn test_debug_pipeline_pause_resume() {
    ...
    debug_set_gmail_poll_paused(initial_state.gmail_poll_paused).await.unwrap();
    debug_set_scan_queue_paused(initial_state.scan_queue_paused).await.unwrap();
}
...
async fn test_debug_fetch_unprocessed_statements() {
    ...
    let stmts = conn.interact(|c| db::unprocessed_statements::select_pending(c)).await.unwrap().unwrap();
...
async fn test_debug_fetch_audit_log() {
    ...
    let logs_all = conn.interact(|c| db::audit_log::fetch_all(c, None, 10, 0)).await.unwrap().unwrap();
    assert_eq!(logs_all.len(), 2);
    
    let logs_filtered = conn.interact(|c| db::audit_log::fetch_all(c, Some("rule".to_string()), 10, 0)).await.unwrap().unwrap();
    assert_eq!(logs_filtered.len(), 1);
    assert_eq!(logs_filtered[0].id, "a1");
}

#[tokio::test]
async fn test_debug_fetch_pattern_rule_health() {
    let pool = setup_test_db().await;
    let conn = pool.get().await.unwrap();
    
    conn.interact(|c| {
        c.execute("INSERT INTO pattern_rules (id, bank_name, template_hash, field_name, rule_payload_json, status, success_count, failure_count) VALUES ('r1', 'HDFC', 'hash1', 'merchant', '{}', 'active', 5, 2)", []).unwrap();
    }).await.unwrap();

    let health = conn.interact(|c| {
        let mut stmt = c.prepare("
            SELECT id, bank_name as merchant_id, 'regex' as pattern_type, field_name as pattern_value, 
        ...
        let iter = stmt.query_map([], |row| {
            Ok(PatternRuleHealth {
                id: row.get(0).unwrap(),
                merchant_id: row.get(1).unwrap(),
                pattern_type: row.get(2).unwrap(),
                pattern_value: row.get(3).unwrap(),
                is_active: row.get(4).unwrap(),
                success_count: row.get(5).unwrap(),
                failure_count: row.get(6).unwrap(),
            })
        }).unwrap();
        let mut res = Vec::new();
        for r in iter { res.push(r.unwrap()); }
    ...
    assert_eq!(health[0].id, "r1");
    assert_eq!(health[0].merchant_id, "HDFC");
    assert_eq!(health[0].success_count, 5);
