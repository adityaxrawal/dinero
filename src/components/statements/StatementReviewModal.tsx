...
export default function StatementReviewModal() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    API.statements.getDraftPdf(activeDraftId)
      .then((base64) => {
        if (cancelled) return;
      ...
      .catch(() => {
        if (cancelled) return;
        toast({ title: 'PDF unavailable', description: "This statement's PDF could not be loaded.", variant: 'destructive' });
      });
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    API.statements.getDraft(activeDraftId).then((d) => {
      if (cancelled) return;
      setDraft(d);
      setMetadata({
  ...
  const updateRow = useCallback((index: number, field: keyof DraftRow, value: string) => {
    setRows((prev) => prev.map((row, i) => {
      ...
      if (field === 'amount_minor') return { ...row, amount_minor: Math.round(parseFloat(value || '0') * 100) };
  ...
  const addRow = useCallback(() => {
    setRows((prev) => [...prev, {
      transaction_date: '', merchant_raw: '', amount_minor: 0, currency: 'INR',
      direction: 'debit', reference_id: null, row_index: prev.length, llm_extracted: false,
    }]);
  }, []);
  ...
  const handleCancel = () => {
    if (activeDraftId) {
      discardDraft.mutate(activeDraftId, {
        onSuccess: () => toast({ title: 'Discarded', description: 'Statement removed from review.' }),
      });
    }
    closeReviewModal();
  };
  ...
  const handleSubmit = () => {
    ...
    commitDraft.mutate({ draftId: activeDraftId, metadata, rows }, {
      onSuccess: () => {
  ...
  return (
    <Dialog open={reviewModalOpen} onOpenChange={(open) => { if (!open) handleCancel(); }}>
      <DialogContent className="sm:max-w-[1200px] w-[95vw] p-0 overflow-hidden flex flex-col max-h-[92vh] h-[820px]" aria-labelledby="review-dialog-title">
        <div className="grid grid-cols-1 md:grid-cols-[1fr_480px] flex-1 min-h-0 h-full">
          ...
          <div className="p-6 flex flex-col h-full overflow-y-auto bg-background">
            <DialogHeader className="mb-4">
              ...
              <DialogDescription>
                {isStaged ? 'Review and correct the extracted data before saving.' : 'Extracting data from your statement…'}
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="flex-1 flex flex-col gap-4 min-h-0">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input id="rm-bank" value={metadata.issuerName} onChange={(e) => setMetadata({ ...metadata, issuerName: e.target.value })} />
                  </div>
                  <div>
                    <Label htmlFor="rm-card">Card number (last 4)</Label>
                    <Input id="rm-card" value={metadata.maskedIdentifier} onChange={(e) => setMetadata({ ...metadata, maskedIdentifier: e.target.value })} />
                  </div>
                  <div>
                    <Label htmlFor="rm-due">Due date</Label>
                    <Input id="rm-due" type="date" value={metadata.dueDate ?? ''} onChange={(e) => setMetadata({ ...metadata, dueDate: e.target.value || null })} />
                  </div>
                  <div>
                    <Label htmlFor="rm-billing-date">Billing date</Label>
                    <Input id="rm-billing-date" type="date" value={metadata.statementDate ?? ''} onChange={(e) => setMetadata({ ...metadata, statementDate: e.target.value || null })} />
                  </div>
                  <div>
                    <Label htmlFor="rm-period-start">Billing cycle start</Label>
                    <Input id="rm-period-start" type="date" value={metadata.billingPeriodStart ?? ''} onChange={(e) => setMetadata({ ...metadata, billingPeriodStart: e.target.value || null })} />
                  </div>
                  <div>
                    <Label htmlFor="rm-period-end">Billing cycle end</Label>
                    <Input id="rm-period-end" type="date" value={metadata.billingPeriodEnd ?? ''} onChange={(e) => setMetadata({ ...metadata, billingPeriodEnd: e.target.value || null })} />
                  </div>
                  <div>
                    <Label htmlFor="rm-balance">Current balance (₹)</Label>
                    <Input id="rm-balance" type="number" value={metadata.currentBalance != null ? metadata.currentBalance / 100 : ''} onChange={(e) => setMetadata({ ...metadata, currentBalance: e.target.value ? Math.round(parseFloat(e.target.value) * 100) : null })} />
                  </div>
                  <div>
                    <Label htmlFor="rm-min-due">Minimum due (₹)</Label>
                    <Input id="rm-min-due" type="number" value={metadata.minimumDue != null ? metadata.minimumDue / 100 : ''} onChange={(e) => setMetadata({ ...metadata, minimumDue: e.target.value ? Math.round(parseFloat(e.target.value) * 100) : null })} />
                  </div>
                </div>
                ...
                <div className="flex-1 min-h-0 flex flex-col">
                  ...
                  <div className="flex-1 overflow-y-auto space-y-2 border rounded-md p-2">
                    {rows.map((row, i) => (
                      <div key={i} className="grid grid-cols-[110px_1fr_90px_70px_28px] gap-1.5 items-center">
                        <Input type="date" value={row.transaction_date} onChange={(e) => updateRow(i, 'transaction_date', e.target.value)} aria-label={`Row ${i + 1} date`} />
                        <Input value={row.merchant_raw} onChange={(e) => updateRow(i, 'merchant_raw', e.target.value)} aria-label={`Row ${i + 1} merchant`} />
                        <Input type="number" value={row.amount_minor / 100} onChange={(e) => updateRow(i, 'amount_minor', e.target.value)} aria-label={`Row ${i + 1} amount`} />
                        <select
                        ...
                        <Button variant="ghost" size="sm" onClick={() => removeRow(i)} aria-label={`Delete row ${i + 1}`}>
            ...
            <DialogFooter className="pt-4 grid grid-cols-2 gap-3">
              <Button variant="outline" onClick={handleCancel} disabled={discardDraft.isPending}>Cancel</Button>
