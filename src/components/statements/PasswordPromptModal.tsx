...
export default function PasswordPromptModal({ onUnlocked }: { onUnlocked: () => void }) {
  ...
  useEffect(() => {
    console.log(
      '[PasswordPromptModal] useEffect triggered. passwordModalOpen:',
      passwordModalOpen,
      'pendingStatementId:',
      pendingStatementId
    );
    if (passwordModalOpen && pendingStatementId) {
      console.log(
        '[PasswordPromptModal] Fetching un-processed statements to find details for statement ID:',
        pendingStatementId
      );
      API.statements
        .listUnprocessed()
        .then((groups) => {
          ...
          if (found) {
            ...
            console.log(
              '[PasswordPromptModal] Could not find matching statement details for ID:',
              pendingStatementId
            );
          }
        })
      ...
      console.log(
        '[PasswordPromptModal] Clearing statement details (modal closed or no statement ID)'
      );
  ...
  const submitPassword = useCallback(async () => {
    ...
    console.log(
      '[PasswordPromptModal] pendingStatementId:',
      pendingStatementId,
      'pendingInstrumentId:',
      pendingInstrumentId
    );
    if (!pendingStatementId || !password.trim()) {
      console.log(
        '[PasswordPromptModal] Validation failed. Missing statement ID or empty password. Aborting submission.'
      );
      return;
    }
    ...
    try {
      ...
      if (result.status === 'unlocked') {
        console.log(
          '[PasswordPromptModal] Status is "unlocked". Opening review modal directly with the returned draft_id.'
        );
        ...
        console.log(
          '[PasswordPromptModal] Status is "awaiting_instrument_confirmation". Closing silently, handing off to Instrument modal.'
        );
        close();
      } else {
        console.log(
          '[PasswordPromptModal] Status is NOT "unlocked" (e.g., incorrect password). Setting error state.'
        );
      ...
      console.log(
        '[PasswordPromptModal] submitPassword() finally block. Setting isSubmitting to false.'
      );
