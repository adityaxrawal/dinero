...
export default function InstrumentInspector({
  ...
  const { data: detailInst, isLoading } = useInstrumentDetail(instrument?.id);
  const { data: txPage } = useTransactionsInfiniteList(instrument ? { instrument_id: instrument.id } : {});
  ...
  const inst = detailInst ?? instrument; // Optimistic display before detail loads
  const isNegative = (inst.current_balance ?? 0) < 0;

  const recentTransactions = txPage?.pages[0]?.records.slice(0, 10) ?? [];
  const instrumentStatements = statements.filter((s) => s.instrument_id === inst.id);
  const instrumentPasswords = pdfPasswords.filter((p) => p.instrument_id === inst.id);
  ...
  return (
    <aside
      ...
      aria-hidden={!isOpen}
      style={inline ? { backgroundColor: '#F8E7C9' } : { width: isOpen ? 'var(--inspector-width)' : 0 }}
    >
        {/* Header */}
        <div
        ...
        >
          <div className="min-w-0 flex-1 pr-3">
            ...
            >
              {isNegative ? '−' : ''}₹{Math.abs(inst.current_balance ?? 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </p>
          </div>
        ...
        {/* Content */}
        <div className={cn("flex-1 overflow-y-auto", inline ? "p-8 max-w-3xl mx-auto w-full" : "p-4")}>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-[#F8E7C9]/50 rounded-xl overflow-hidden border border-[#064E3B]/10">
                    <div className="flex flex-col gap-1 p-3 border-b border-[#064E3B]/5">
                      <Label htmlFor="insp-inst-full-id" className="text-[11px] font-semibold uppercase tracking-wider text-[#064E3B]/60">
                    ...
                    {inst.instrument_type === 'credit_card' && (
                      <div className="flex flex-col gap-1 p-3 border-b border-[#064E3B]/5">
                        <Label htmlFor="insp-inst-billing" className="text-[11px] font-semibold uppercase tracking-wider text-[#064E3B]/60">
                          Billing Cycle Day
                        </Label>
                        <Input
                    ...
                    {inst.instrument_type === 'bank_account' && (
                      <div className="flex flex-col gap-1 p-3 border-b border-[#064E3B]/5">
                        <Label htmlFor="insp-inst-ifsc" className="text-[11px] font-semibold uppercase tracking-wider text-[#064E3B]/60">
                          IFSC Code
                        </Label>
                        <Input
                          id="insp-inst-ifsc"
                  ...
                  {instrumentPasswords.length > 0 && (
                    <div className="space-y-2 pt-2">
                      <p className="text-[11px] font-semibold uppercase tracking-wider text-[#064E3B]/60">Saved Passwords</p>
                      {instrumentPasswords.map((p) => (
                        ...
                        >
                          ...
                          <button
                            type="button"
                            onClick={() => handleForgetPassword(p.id)}
              ...
              {activeTab === 'transactions' && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-medium" style={{ color: '#6b8a7f' }}>Recent Activity</span>
                    <button
                      type="button"
                  ...
                  {recentTransactions.length === 0 ? (
                    <p className="text-xs text-center py-6" style={{ color: '#6b8a7f' }}>No transactions found.</p>
                  ) : (
                      ...
                      {recentTransactions.map((tx) => (
                        ...
                        >
                          <div className="min-w-0 pr-2">
                            <p className="text-xs font-medium truncate" style={{ color: 'var(--text-primary)' }}>{tx.merchant}</p>
                            <p className="text-[10px]" style={{ color: '#6b8a7f' }}>{formatCustomDate(tx.date)}</p>
                          </div>
                          <span
                            className={cn('text-xs font-semibold amount flex-shrink-0', tx.amount < 0 ? 'amount-debit' : 'amount-credit')}
                          >
              ...
              {activeTab === 'statements' && (
                <div className="space-y-3">
                  {instrumentStatements.length === 0 ? (
                    <p className="text-xs text-center py-6" style={{ color: '#6b8a7f' }}>No statements uploaded.</p>
                  ) : (
                    <div className="space-y-2">
                      {instrumentStatements.map((s) => (
                        ...
                        >
                          <div className="flex items-start justify-between">
                            <span className="text-xs font-medium truncate pr-2" style={{ color: 'var(--text-primary)' }} title={s.file_name}>
                              {s.file_name}
                            </span>
                            <span
                              className="text-[10px] font-medium px-2 py-0.5 rounded-full flex-shrink-0"
                              style={{
                                background: s.status === 'PROCESSED' ? 'rgba(16,185,129,0.10)' : 'rgba(107,138,127,0.10)',
                          ...
                          <p className="text-[10px]" style={{ color: '#6b8a7f' }}>Uploaded {formatCustomDate(s.date)}</p>
                        </div>
                      ))}
                    </div>
                  )}
