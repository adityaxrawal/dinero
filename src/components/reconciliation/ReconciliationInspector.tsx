...
export default function ReconciliationInspector({
  ...
  const asideStyle = inline ? { backgroundColor: '#F8E7C9' } : { width: (cluster && isOpen) ? 'var(--inspector-width)' : 0 };
        ...
        >
          <div className="min-w-0 flex-1 pr-3">
            ...
            <p className="text-[15px] font-semibold text-[#064E3B]">
              {cluster.reason.startsWith('Ambiguous match') ? cluster.reason.substring(15).trim() : cluster.reason}
            </p>
          </div>
        ...
        <div className={cn("flex-1 overflow-y-auto flex flex-col", inline ? "p-8 max-w-3xl mx-auto w-full" : "p-4")}>
          ...
          <div className="flex-1">
            ...
            {isLoading && !detailCluster ? (
              <div className="flex items-center justify-center py-12 text-sm gap-2" style={{ color: '#6b8a7f' }}>
                Loading details…
              </div>
            ) : (
          ...
          <div 
            className="mt-6 p-4 rounded-xl border border-[#064E3B]/10 flex flex-col gap-3 bg-[#F8E7C9]/50"
          >
