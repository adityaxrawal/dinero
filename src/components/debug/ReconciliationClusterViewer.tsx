        ...
        <tr key={cluster.id} className="border-b border-[var(--border-color)] last:border-0">
          ...
          <td className="p-2 text-sm">
            {cluster.status === 'resolved' ? (
               <Badge variant="outline" className="bg-green-500/10 text-green-700">Resolved</Badge>
            ) : (
               <Badge variant="outline" className="bg-yellow-500/10 text-yellow-500">{cluster.status}</Badge>
            )}
          </td>
          <td className="p-2 text-sm font-mono">
            {cluster.total_amount_minor != null ? (cluster.total_amount_minor / 100).toFixed(2) : '-'} {cluster.currency}
          </td>
          <td className="p-2 text-sm font-mono text-muted-foreground">{cluster.observation_id?.substring(0, 8)}</td>
        </tr>
      )}
    />
  );
