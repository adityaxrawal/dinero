      ...
      <Card className="w-full max-w-2xl shadow-2xl">
        <CardHeader>
          ...
          <CardDescription className="mt-2">
            Financial data never leaves your Mac. The app makes network calls only for the purposes listed
            below, each strictly limited in scope.
          </CardDescription>
        </CardHeader>

        <CardContent>
          <div className="border border-border rounded-lg overflow-hidden" role="table" aria-label="Network communication disclosure">
            <div
            ...
            {NETWORK_DISCLOSURE_TABLE.map((row) => (
              <div
                ...
                role="row"
              >
                <span className="font-medium" role="cell">{row.destination}</span>
                <span className="text-muted-foreground text-xs" role="cell">{row.dataSent}</span>
                <span className="text-muted-foreground text-xs" role="cell">{row.when}</span>
              </div>
            ))}
          </div>
          ...
          <div className="mt-3 flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>Financial data — transactions, balances, statements — is never sent for any of the above. No third-party analytics or crash-reporting services are used.</span>
          </div>
        </CardContent>

        <CardFooter className="justify-end">
          <Button onClick={onContinue} variant="accent" aria-label="Continue past the network disclosure">
            Continue
          </Button>
        </CardFooter>
      </Card>
