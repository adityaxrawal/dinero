...
function App() {
  useEffect(() => {
    ...
    const syncLlmState = async () => {
      try {
        const hw = await API.llm.getHardwareInfo();
        const models = await API.llm.getAvailableModels();
        if (models.length === 0) return;

        // 1. Sync Model
        let selectedId = localStorage.getItem('llm_model') || hw.recommended_model_id || models[0].id;
        const selected = models.find((m) => m.id === selectedId);
        const override = localStorage.getItem('llm_ram_override') === 'true';

        if (selected && hw.ram_gb < selected.min_ram_gb && !override) {
          if (
            window.confirm(
          ...
          localStorage.setItem('llm_model', selectedId);
        }
        await API.llm.setActiveModel(selectedId);

        // 2. Sync Parallel Slots
        const storedSlots = localStorage.getItem('llm_parallel_slots');
        const initialSlots = storedSlots ? Math.min(10, Math.max(1, Math.round(parseInt(storedSlots, 10)))) : hw.recommended_slots;
        await API.llm.setParallelSlots(initialSlots);
        localStorage.setItem('llm_parallel_slots', initialSlots.toString());
      } catch (e) {
