...
export interface FormattedMerchantInfo {
  cleanName: string;
...
export function formatMerchantName(rawName: string): string {
  ...
  if (lower.includes('blinkit') || lower.includes('blinkmerc')) return 'Blinkit';
  if (lower.includes('flipkart')) return 'Flipkart';
  if (lower.includes('amazon')) return 'Amazon';
  if (lower.includes('airtel') || lower.includes('bhartiairtel')) return 'Airtel';
  ...
  if (lower.includes('make my trip') || lower.includes('makemytrip')) return 'MakeMyTrip';
 ...
 */
export function getMerchantCategoryVisuals(category?: string, merchantName?: string): {
  ...
  // Food & Dining
  if (cat.includes('food') || cat.includes('dining') || name.includes('zomato') || name.includes('swiggy')) {
    return {
  ...
  if (cat.includes('grocer') || name.includes('blinkit') || name.includes('zepto') || name.includes('instamart')) {
    return {
  ...
  if (cat.includes('finance') || cat.includes('card') || name.includes('cred') || name.includes('dreamplug')) {
    return {
  ...
  // Travel / Commute
  if (cat.includes('travel') || cat.includes('cab') || name.includes('uber') || name.includes('ola')) {
    return {
  ...
  // Entertainment
  if (cat.includes('entertain') || name.includes('netflix') || name.includes('bookmyshow') || name.includes('spotify')) {
    return {
